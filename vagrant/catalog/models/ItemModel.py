...
def get_item(item_name, category_id):
    '''
    get item using item name & category id
    '''
...
def get_lastest_items():
    '''
    get 5 latest items added to database
    from all categories
    '''
    catalog_id = CatalogModel.get_catalog_id()
    items = session.query(Item).filter_by(
        catalog_id=catalog_id).order_by(Item.id.desc()).limit(5).all()
    return items


def get_user_items(user_id):
    '''
    get items of a user by using user id
    '''
    user_id = int(user_id)
    catalog_id = CatalogModel.get_catalog_id()
    items = session.query(Item).filter_by(
        catalog_id=catalog_id, user_id=user_id).all()
    return items


def get_category_items(category_id):
    '''
    get category items using category id
    '''
    category_id = int(category_id)
    catalog_id = CatalogModel.get_catalog_id()
    items = session.query(Item).filter_by(
        catalog_id=catalog_id, category_id=category_id).all()
    return items
